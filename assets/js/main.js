...
// Erstelle eine Funktion, die einen Text in einem Abstand von 100 Zeichen 
// trennt.
// Füge nach den 100 Zeichen “ - 1 of gesamte-Nachrichten”, “ - 2 of
// gesamte-Nachrichten” usw … hinzu.
// Jedes Element dieser Zeichenkette (100 Zeichen + - 1 of x ) soll in ein
//  Array gepusht werden.
// Gib dir am Ende das Ergebnis in der Konsole aus.
// Rufe die Funktion mit der Variablen als Argument auf.
show('CodeFlow Übung lev3_1: for loop string Array', 'h2');
...
function splitText(maxLength) {
    let maxParts = Math.floor(text.length / maxLength) + (text.length % maxLength == 0 ? 0 : 1);
    let parts = [];
    for (let i = 0; i < maxParts; i++) {
        let part = text.substr(i * maxLength, maxLength);
        part += ` - ${i + 1} of ${maxParts}`;
        parts.push(part);
    }
    return parts;
}
splitText(100).forEach(part => show(part));
show();


//****************************************************************************
// CodeFlow Übung lev3_2: for loop Zahlen in Array teilen
// Du hast ein Array aus Zahlen und möchtest herausfinden, durch welche Zahlen
// sie (außer durch eins und sich selbst) teilbar sind.
// Schreibe das Ergebnis in dein HTML.
show('CodeFlow Übung lev3_2: for loop Zahlen in Array teilen', 'h2');
